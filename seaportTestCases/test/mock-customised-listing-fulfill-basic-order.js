...
mockAddress1 = "0x724d74374F083e94c72f01733f66e8cd77Dd4d82";
...
async function printBalances(sellerAddr, buyerAddr) {
  console.log("\nseller's balance:", await provider.getBalance(sellerAddr));
  console.log("\nbuyer's balance:", await provider.getBalance(buyerAddr));
  // domain balances
...
describe("Seaport main interfaces", function () {
  ...
  it("customised listing by `seller` and fulfill order by `buyer`", async () => {
    ...
    // call by seaport contract
    const price1 = await provider.getGasPrice();
    console.log("当前gas价格: ", price1, "wei");
    const options1 = {
      gasPrice: price1.mul(2),
      value: "100000000000000000",
      gasLimit: 13000000,
    }; // 防止失败，gasPrice = currentGasPrice * 1.2
    ...
    // const price1 = await provider.getGasPrice();
    // console.log("当前gas价格: ", price, "wei");
    // const options1 = { gasPrice: price1.mul(2), gasLimit: 1300000 }; // 防止失败，gasPrice = currentGasPrice * 1.2
    // const transaction = await action.transactionMethods.transact(options1);
