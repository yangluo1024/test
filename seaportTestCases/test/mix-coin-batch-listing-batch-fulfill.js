...
const SEA_ADDRESS = '0x00000000000001ad428e4906aE43D8F9852d0dD6'
...
const UD_TOKEN_ID = [
	...
	'75571939348141571573239362568227921804799590306738988475638704135483441350151',
...
describe('Seaport main interfaces', function () {
	...
	it('卖家不同币种上架多个商品，被buyer混合支付批量购买', async () => {
		...
		// ======================================
		seaport.config.seaportVersion = '1.4'
		const { executeAllActions } = await seaport.createBulkOrders(orders)
		const orderInfos = await executeAllActions()
		// ======================================
		...
		// 将普通order优化为advanced order
		for (let i = 0; i < orderInfos.length; i++) {
			orderInfos[i].numerator = 1
			orderInfos[i].denominator = 1
			orderInfos[i].extraData = '0x'
		}
