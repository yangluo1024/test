// BID只能使用ETH, 将WETH视为商品, BID并由seller确认卖出
...
describe("Seaport main interfaces", function () {
      ...
      offer: [
        {
          ...
          token: WETH_ADDRESS,
          identifier: "0",
          amount: orderAmount, // bid时，offer中amount就是订单价
        },
      ],
      consideration: [
        {
  ...
  it("buyer使用ETH BID，并由seller确认卖出", async () => {
    ...
    // seller授权seaport指定合约使用所有ud域名
    await udContract
      .connect(seller)
      .setApprovalForAll("0x1e0049783f008a0085193e00003d00cd54003c71", true);

    // TODO: 此处直观来讲，应该是buyer授权给seaport来转移weth, 但由于是由seller发起交易，居然是要seller授权seaport使用weth？
    // seller授权给seaport指定合约, 此合约地址固定为: 0x1e0049783f008a0085193e00003d00cd54003c71
    await wethContract
      .connect(seller)
    ...
    // BID订单中，由seller充当fulfiller来调用合约进行交易
