...
// 打印seller和buyer的域名数, 以及seller和feeer的matic
async function printBalances(sellerAddr, buyerAddr, feeAddr) {
  ...
  const buyerBalance = await provider.getBalance(buyerAddr);
  console.log("buyer余额:", buyerBalance);
  const feeerBalance = await provider.getBalance(feeAddr);
  console.log("feeer余额:", feeerBalance);
...
describe("Seaport main interfaces", function () {
  beforeEach(async function () {
    ...
    // 将费用定义成变量，方便后面验证
    orderAmount = "100000000000000000"; // 订价0.1 matic
    handlingFee = "10000000000000000"; // 手续费, 10％
    // 订单信息, 两个订单
    orders = [
      {
        offer: [
  ...
  it("seller使用MATIC上架订单，被buyer批量购买", async () => {
    // 交易前buyer和feeAddr的matic余额，以及seller和buyer的域名数
    ...
    // 调用合约进行交易
    const options = { value: (parseInt(orderAmount) * 2).toString() };
    const tx = await seaportContract.connect(buyer).fulfillAvailableOrders(
      orderInfos,
      [[[0, 0]], [[1, 0]]],
      [
        [
    ...
    // seller域名减2， buyer域名增加2
    expect(balancesBefore[0].sub(balancesAfter[0]).toString()).to.equal("2");
    expect(balancesAfter[1].sub(balancesBefore[1]).toString()).to.equal("2");
    // feeer's matic增加应等于订单的10%, 即handlingFee
